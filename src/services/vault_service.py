...
from src.helpers.cipher import encrypt, decrypt
...
class VaultService:
    ...
    async def update_vault(self, vault_id: str, user_id: str, title: str, username: str, password: str, key: str) -> MessageResponse:
        encrypted_pw = encrypt(password, key)
        matched = await self._repo.update(vault_id, user_id, {
            "title": title,
            "username": username,
            "password": encrypted_pw,
            "updatedAt": datetime.now(timezone.utc),
